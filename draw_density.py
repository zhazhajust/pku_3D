...
def processplot(n): 
  #for n in range(start,stop+step,step):
    ...
    data=sdf.read('../Data/a2_n1_T6_w7/2200.sdf',dict=True)
    ...
    print(x)
    var1  = data['Derived/Number_Density/electron1'].data
...
#    print('here2')
    ...
    if cut_quarter:
        ...
        im = ax.scatter(X, Y, Z,s=0.1, c=var, norm=colors.Normalize(vmin=-eee,vmax=eee), cmap=cmap_my_laser2, **plotkws)
#    print('here3')
...
#    cbar.set_label(name+r'$[m_ec\omega/|e|]$',fontdict=font)
    ...
    ex[ex>eee] = eee
    ex[ex<-eee] = -eee
    #ax.contourf(X.T, ex.T, Z.T, levels=levels, norm=mcolors.Normalize(vmin=-eee, vmax=eee), cmap=cmap_my_bwr, zdir='y', offset=250)
#    ax.set_xlim([x_start/20-5,x_stop/20-5])
...
#    ax.set_ylim([-(z_stop-z_start)/2/12,(z_stop-z_start)/2/12])
    ...
    ex[ex>eee] = eee
    ex[ex<-eee] = -eee
    #im2=ax.contourf(X.T, Y.T, ex.T, levels=levels, norm=mcolors.Normalize(vmin=-eee, vmax=eee), cmap=cmap_my_bwr, zdir='z', offset=-250.0)
#    ax.set_xlim([x_start/20-5,x_stop/20-5])
#    ax.set_ylim([-(y_stop-y_start)/2/12,(y_stop-y_start)/2/12])
...
#    ax.grid(False)
  ...
  #  plt.title('At '+str(round(time/1.0e-15,2))+' fs',fontdict=font)
    ...
    fig.savefig('./fig/3d_density.png',format='png',dpi=80)
    plt.close("all")
